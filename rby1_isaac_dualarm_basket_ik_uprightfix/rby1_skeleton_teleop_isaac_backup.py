...
parser.add_argument("--human-gain", type=float, default=0.75, help="Scale from human wrist displacement to robot EE displacement.")
...
class ArmTargetMapper:
    ...
    def target(self, pkt: dict, side: str, workspace_min: np.ndarray, workspace_max: np.ndarray) -> np.ndarray | None:
        ...
        raw = self.robot_neutral[side] + self.gain * (feat - self.human_neutral[side])
...
def main():
    ...
    while simulation_app.is_running():
        ...
        dq_left += 0.025 * (q_left_ready - q_left)
        dq_right += 0.025 * (q_right_ready - q_right)
