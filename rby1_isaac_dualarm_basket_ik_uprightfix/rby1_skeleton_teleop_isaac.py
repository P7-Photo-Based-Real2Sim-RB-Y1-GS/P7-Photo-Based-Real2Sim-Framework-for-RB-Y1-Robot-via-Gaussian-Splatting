...
parser.add_argument("--center-pull", type=float, default=0.35, help="Pull both hand targets toward the robot/task center in y direction.")
parser.add_argument("--center-y", type=float, default=0.0, help="Task center y position for grasping.")
parser.add_argument("--wrist-pull", type=float, default=0.35, help="Additional wrist joint regularization strength.")
parser.add_argument("--ee-orient-lock", action="store_true", help="Keep end-effector orientation near the calibrated neutral orientation.")
parser.add_argument("--orient-gain", type=float, default=0.20)
parser.add_argument("--orient-weight", type=float, default=0.35)
...
def _get_world_quat(view: XFormPrim) -> np.ndarray:
    _pos, quat = view.get_world_poses()
    q = _as_np(quat[0])
...
def _quat_error_vec(q_des: np.ndarray, q_cur: np.ndarray) -> np.ndarray:
    # Isaac uses quaternion order [w, x, y, z].
...
def _dls_step6(jacobian_6d: np.ndarray, err_6d: np.ndarray, damping: float, gain: float) -> np.ndarray:
    j = jacobian_6d
    a = j @ j.T + (damping ** 2) * np.eye(6)
    return j.T @ np.linalg.solve(a, gain * err_6d)
...
class ArmTargetMapper:
    ...
    def target(self, pkt: dict, side: str, workspace_min: np.ndarray, workspace_max: np.ndarray) -> np.ndarray | None:
        ...
        # CENTER_PULL_PATCH:
        # Pull hand target toward the task center so the gripper can reach the cup/basket area.
        # Isaac convention is approximately x-forward, y-left/right, z-up.
        raw[1] = (1.0 - args_cli.center_pull) * raw[1] + args_cli.center_pull * args_cli.center_y
...
def main():
    ...
    left_neutral_quat = None
    right_neutral_quat = None
    ...
    while simulation_app.is_running():
        ...
        if tracking:
            was_calibrated = mapper.calibrated
            mapper.update_calibration(pkt, left_pos, right_pos)
            if (not was_calibrated) and mapper.calibrated:
                left_neutral_quat = _get_world_quat(left_ee_view)
                right_neutral_quat = _get_world_quat(right_ee_view)
                print("[CALIB] captured neutral EE orientations for wrist stabilization.")
        ...
        if target_left is not None:
            err_l = _clip_norm(target_left - left_pos, 0.12)
            if args_cli.ee_orient_lock and left_neutral_quat is not None:
                qcur_l = _get_world_quat(left_ee_view)
                eori_l = args_cli.orient_weight * _quat_error_vec(left_neutral_quat, qcur_l)
                eori_l = _clip_norm(eori_l, 0.35)
                j_l = jac[0, left_body_idx, 0:6, :][:, left_cols]
                err6_l = np.concatenate([err_l, args_cli.orient_gain * eori_l])
                dq_left = _dls_step6(j_l, err6_l, args_cli.lambda_dls, args_cli.ik_gain)
            else:
        ...
        if target_right is not None:
            err_r = _clip_norm(target_right - right_pos, 0.12)
            if args_cli.ee_orient_lock and right_neutral_quat is not None:
                qcur_r = _get_world_quat(right_ee_view)
                eori_r = args_cli.orient_weight * _quat_error_vec(right_neutral_quat, qcur_r)
                eori_r = _clip_norm(eori_r, 0.35)
                j_r = jac[0, right_body_idx, 0:6, :][:, right_cols]
                err6_r = np.concatenate([err_r, args_cli.orient_gain * eori_r])
                dq_right = _dls_step6(j_r, err6_r, args_cli.lambda_dls, args_cli.ik_gain)
            else:
        ...
        # WRIST_PULL_PATCH_LEFT: keep wrist joints near ready posture to avoid excessive wrist bending.
        if len(dq_left) >= 2:
            dq_left[-2:] += args_cli.wrist_pull * (q_left_ready[-2:] - q_left[-2:])
        dq_right += args_cli.posture_pull * (q_right_ready - q_right)
        # WRIST_PULL_PATCH_RIGHT: keep wrist joints near ready posture to avoid excessive wrist bending.
        if len(dq_right) >= 2:
            dq_right[-2:] += args_cli.wrist_pull * (q_right_ready[-2:] - q_right[-2:])
